...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".notclaude.json"),
            |parent| parent.join(".notclaude.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".notclaude.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".notclaude").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".notclaude").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("NOTCLAUDE_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".notclaude")))
        .unwrap_or_else(|| PathBuf::from(".notclaude"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".notclaude.json");
...
mod tests {
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".notclaude");
    ...
    fn loads_and_merges_claw_code_config_files_by_precedence() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".notclaude");
        fs::create_dir_all(cwd.join(".notclaude")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".notclaude.json"),
        ...
        fs::write(
            cwd.join(".notclaude.json"),
            r#"{"model":"project-compat","env":{"B":"2"}}"#,
        )
        .expect("write project compat config");
        fs::write(
            cwd.join(".notclaude").join("settings.json"),
            r#"{"env":{"C":"3"},"hooks":{"PostToolUse":["project"]},"mcpServers":{"project":{"command":"uvx","args":["project"]}}}"#,
        )
        .expect("write project settings");
        fs::write(
            cwd.join(".notclaude").join("settings.local.json"),
